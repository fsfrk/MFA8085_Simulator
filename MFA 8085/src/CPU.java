...
public class CPU implements ActionListener , WindowListener
{
	...
	Befehlsdatenbank befehl_DB =new Befehlsdatenbank();				//Alle Allgemeine angaben zu den möglichen Befehlen
	private Map <String,Byte>baugruppenspeicher    =new HashMap<String,Byte>();
	public  Map <String,Byte>reg  =new HashMap<String,Byte>();		//A,B,C,D,E,H,L,SP,FLAG = Flagregister: C=Carry;A=Auxiliary;Z=Zero;S=Sign-Flag;P=Parität;O=Overflow
	...
	public static void main(String[] args)	{new CPU();}
...
/////////////////////////////add Abschnitt//////////////////////////////////////////////////////////////
	...
	/*public void setLabels(byte value)
	{
		...
		{
			if((1<<i&value)>0)
				speicher[i].setBackground(Color.green);
			else
	...
	public byte in(byte port)
	{
		...
		short  wert=0;
		for(byte i=7 ; i>=0 ; i--)
			wert=(short)(this.key.get("IN: 40 "+i).getBackground().equals(Color.green) ? wert<<1|1 : wert<<1);
		return (byte)wert;
	}
...
/////////////////////////////Listener Abschnitt/////////////////////////////////////////////////////////
	@Override public void actionPerformed(ActionEvent e) 
	{
		...
		if(e.getActionCommand().equals("Rechner"))	{	GUI.rechner(this);}
		if(e.getActionCommand().equals("umrechnen"))
		{
			...
			if(sp[0]==null || sp[1]==null || sp[2]==null)
				GUI.errormessage("ERROR, the Label hex,dec,bin doesn't exist",false);
			if(sp[0].getText().length()>0 && sp[1].getText().length()>0 ||
					...
					GUI.errormessage("Es müssen 2 Felder auf 0 sein",false);
			else
			{
				try
				{
					...
					{
						sp[0].setText(Integer.toHexString(Integer.parseInt(sp[2].getText(),2)));
						sp[1].setText(""+Integer.parseInt(sp[2].getText(),2));
					}
					if(sp[0].getText().length()>0)
					{
						sp[1].setText(""+Integer.parseInt(sp[0].getText(),16));
						sp[2].setText(Integer.toBinaryString(Integer.parseInt(sp[0].getText(),16)));
					}
					if(sp[1].getText().length()>0)
					{
						sp[0].setText(Integer.toHexString(Integer.parseInt(sp[1].getText())));
						sp[2].setText(Integer.toBinaryString(Integer.parseInt(sp[1].getText())));
					}
				}
				catch(NumberFormatException n)
				{
					GUI.errormessage("Wert zu groß: "+n.getMessage(),false);
				}
			}
		}
		if(e.getActionCommand().equals("IN: 40"))
		{
			for(byte i=0 ; i<8 ; i++)
			{
				if(this.key.get("IN: 40 "+i).equals(e.getSource()))
				{
					if(key.get("IN: 40 "+i).getBackground().equals(Color.red))
					{
					...
					this.baugruppenspeicher.put("in",(byte) (this.baugruppenspeicher.get("in")^(1<<i)));
				}
		...
		if(e.getSource().equals(this.m_item.get("interpreter")))
		{
			...
			else
			{
				...
				this.interpreter= new Interpreter(this.befehl_DB , this.area.get("interpreter").getText(),this,this.m_item.get("interpreter"));
